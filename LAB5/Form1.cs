    ...
    public partial class Form1 : Form
    {
        ...
        private void CalculateButton_Click(object sender, EventArgs e)
        {
            ...
            if (!Debug)
            {
                try
                {
                    Start = Convert.ToDouble(StartABox.Text.Replace('.', ','));
                }
                catch
                {
                ...
                try
                {
                    End = Convert.ToDouble(EndBBox.Text.Replace('.', ','));
                }
                catch
                {
                ...
                {
                    MessageBox.Show("Ошибка формата числа a. Число должно быть в диапазоне от -1000000 до 1000000");
                    return;
                }

                if (End < -1000000 || End > 1000000)
                {
                    MessageBox.Show("Ошибка формата числа b. Число должно быть в диапазоне от -1000000 до 1000000");
                    return;
                }
            }
        ...
        void Calculate(string Function, double Start, double End, int StepsQuantity)
        {
            ...
            AnswerRectangle.Text = RectanglesValue.ToString();
            AnswerTrapeze.Text = TrapezoidsValue.ToString();
            AnswerParabola.Text = ParabolasValue.ToString();
