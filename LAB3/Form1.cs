...
namespace LeastSquareMethod
{
    public partial class Form1 : Form
    {
        ...
        List<List<double>> GetAllPoints()
        {
            ...
            for (int RowCount = 0; RowCount < GridTable.RowCount; RowCount++)
            {
                try
                {
                    ...
                    {
                        var Row = GridTable.Rows[RowCount];
        ...
        private void CalculateButton(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Function.Text = $"Аппроксимировать невозможно!";
                }
            }
            catch
            {
        ...
        private void QuadraticCalculate(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Double.IsNaN(Coefficients[0]) && !Double.IsNaN(Coefficients[1]) && !Double.IsNaN(Coefficients[2]))
                {
                    ...
                    Function.Text = $"Формула: {Coefficients[0]}x² + {Coefficients[1]}x + {Coefficients[2]}";
                }
                else
                {
                    Function.Text = $"Аппроксимировать невозможно!";
